...
axs[1].format(title=L"$\mu$ (REP-RE5) / $10^{-2}$ K",coast=true)

c = axs[2].contourf(
    lon,lat,(Tm_tot.-R5_tot)'*100,
    cmap="RdBu_r",levels=vcat(-5:-1,-0.5,0.5,1:5),extend="both"
)
axs[2].format(title=L"$\delta_t$ (REP-RE5) / $10^{-2}$ K",coast=true)

c = axs[3].contourf(
    lon,lat,(Tm_sea.-R5_sea)'*100,
    cmap="RdBu_r",levels=vcat(-5:-1,-0.5,0.5,1:5),extend="both"
)
axs[3].format(title=L"$\delta_s$ (REP-RE5) / $10^{-2}$ K",coast=true)

c = axs[4].contourf(
    lon,lat,(Tm_itr.-R5_itr)'*100,
    cmap="RdBu_r",levels=vcat(-5:-1,-0.5,0.5,1:5),extend="both"
)
axs[4].format(title=L"$\delta_i$ (REP-RE5) / $10^{-2}$ K",coast=true)

c = axs[5].contourf(
    lon,lat,(Tm_dhr.-R5_dhr)'*1000,
    cmap="RdBu_r",levels=vcat(-5:-1,-0.5,0.5,1:5),extend="both"
)
axs[5].format(title=L"$\delta_d$ (REP-RE5) / $10^{-3}$ K",coast=true)

c = axs[6].contourf(
    lon,lat,(Tm_ian.-R5_ian)'*1000,
    cmap="RdBu_r",levels=vcat(-5:-1,-0.5,0.5,1:5),extend="both"
)
axs[6].format(title=L"$\delta_a$ (REP-RE5) / $10^{-3}$ K",coast=true)
