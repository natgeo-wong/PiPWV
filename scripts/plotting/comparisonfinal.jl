...
axs[2].format(title=L"Linear")

axs[3].contourf(lon,lat,(Tm_avg.-R5_avg)',cmap="RdBu_r",levels=dlvls,extend="both")
axs[3].format(title=L"Linear - ERA5 $T_m$")

c = axs[4].contourf(lon,lat,R5_tot',levels=(1:10)*10,extend="both")
...
# axs[10].colorbar(c,loc="l")

c = axs[13].contourf(lon,lat,R5_dhr',levels=(1:10),extend="both")
...
# axs[13].colorbar(c,loc="l")

c = axs[16].contourf(lon,lat,R5_ian',levels=(1:10),extend="both")
